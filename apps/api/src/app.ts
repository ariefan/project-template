...
export async function buildApp(config: AppConfig) {
  ...
  const { eventBroadcaster, connectionManager } = await setupRealtime(app);

  // Initialize notification system
  await setupNotifications(app, eventBroadcaster);

  // Register WebSocket and SSE plugins
  if (env.REALTIME_ENABLED && eventBroadcaster && connectionManager) {
    await setupRealtimePlugins(app, eventBroadcaster, connectionManager);
  }

  await registerGlobalJobHandlers();

  // Initialize queues
  await setupQueues(app);
...
async function setupRealtime(app: import("fastify").FastifyInstance) {
  ...
  return { eventBroadcaster, connectionManager };
}

async function setupNotifications(
  app: import("fastify").FastifyInstance,
  eventBroadcaster: import("@workspace/realtime").EventBroadcaster | null
) {
  // Initialize notification system
...
async function setupRealtimePlugins(
  app: import("fastify").FastifyInstance,
  eventBroadcaster: import("@workspace/realtime").EventBroadcaster,
  connectionManager: import("@workspace/realtime").ConnectionManager
) {
...
async function registerGlobalJobHandlers() {
...
async function setupQueues(app: import("fastify").FastifyInstance) {
...
async function setupApiDocs(app: import("fastify").FastifyInstance) {
  // ─────────────────────────────────────────────────────────────
  // API Documentation
  // ─────────────────────────────────────────────────────────────
...
async function registerModules(app: import("fastify").FastifyInstance) {
  // Modules
  await app.register(healthRoutes);
  await app.register(developerRoutes, { prefix: "/v1" });
