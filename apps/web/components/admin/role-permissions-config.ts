...
/**
 * Resources derived from @workspace/authorization (single source of truth)
 * @see packages/authorization/src/types.ts
 */
export const RESOURCES = Object.values(AUTH_RESOURCES).map((key) => ({
  key,
  label: RESOURCE_LABELS[key as Resource],
}));
