...
import { listSystemUsers } from "@/actions/system-users";
...
export function UsersList() {
  ...
  // Fetch all users via custom system-users API to get memberships
  const { data: usersData, isLoading: usersLoading } = useQuery({
    queryKey: ["system-users", searchQuery],
    queryFn: async () => {
      const result = await listSystemUsers({ search: searchQuery });
      if (result.error) {
        throw new Error(result.error);
      }
      return { users: result.data || [], total: result.data?.length || 0 };
    },
  });

  const userList = (usersData?.users ?? []) as SystemUser[];
  ...
  const columns = useMemo<ColumnDef<SystemUser>[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        header: "Role",
        headerTooltip:
          "Controls access to this Admin area. Not the same as organization-specific roles (Owner/Admin/Member).",
        cell: ({ row }) => (
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-wrap gap-1">
            {row.organizations && row.organizations.length > 0 ? (
              row.organizations.map((org) => (
                <Badge
                  className="border-none bg-primary/10 px-1.5 py-0 text-[10px] text-primary hover:bg-primary/20"
                  key={org.id}
                  variant="outline"
                >
                  {org.name}
