...
export type { ImageUploaderProps, ImageUploaderRef } from "./types";
...
function EmptyDropzone({
  ...
  const acceptStr = accept ? Object.values(accept).flat().join(",") : "image/*";
...
interface CompactImageItemProps {
  item: CombinedItem;
  compactExpanded: boolean;
  controller: Controller;
}

function CompactImageItem({
  item,
  compactExpanded,
  controller,
}: CompactImageItemProps) {
  const src = item.type === "url" ? item.data.url : item.data.preview;
  ...
  return (
    ...
    >
      {/* biome-ignore lint/performance/noImgElement: Previews are short-lived Blobs or dynamic URLs, not suitable for next/image */}
      <img
        alt="Preview"
        className="size-full object-cover"
        height={compactExpanded ? 80 : 28}
        src={src ?? ""}
        width={compactExpanded ? 80 : 28}
      />
        ...
        <button
          ...
          onClick={(e) => {
            ...
            controller.setPreviewName(
              item.type === "file" ? item.data.name : "Image"
            );
            controller.setPreviewType(
              item.type === "file" ? item.data.type : "image/jpeg"
            );
...
function CompactUploadView({
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {controller.combinedItems.map((item) => (
          <CompactImageItem
        ...
        <button
          ...
          type="button"
        >
          <Trash2 className={compactExpanded ? "size-4" : "size-3"} />
        </button>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <h3 className="font-medium text-xs">
                Images (
                {(initialUrls?.length || 0) + controller.compressedFiles.length}
                )
              </h3>
              {controller.compressedFiles.length > 0 && (
                <Button
...
>((props, ref) => {
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      {/* Main Upload Area Selection */}
      {(() => {
        if (!controller.hasAnyFiles) {
          return (
            <EmptyDropzone
              accept={props.accept}
